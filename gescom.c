...
int analyseCom(char *b)
{
    ...
    if (Mots == NULL) {
        ...
        for (i = 0; i < NBMAXMOTS; i++) Mots[i] = NULL;
    }
...
static int appliquerRedirections(void)
{
    ...
    while (i < NMots) {
        if (strcmp(Mots[i], "<") == 0 || strcmp(Mots[i], ">") == 0 ||
            ...
            free(Mots[i]);
            free(Mots[i+1]);
            for (j = i; j < NMots - 2; j++) Mots[j] = Mots[j+2];
            Mots[NMots - 2] = NULL;
            Mots[NMots - 1] = NULL;
            NMots -= 2;
        } else {
    ...
    for (i = 0; i < num_cmds; i++) {
        ...
        if (pids[i] == 0) {
            ...
            if (analyseCom(cmds[i]) > 0) {
                if (appliquerRedirections() == 0) {
                    if (execComInt(NMots, Mots)) exit(0);
                    execvp(Mots[0], Mots);
                    fprintf(stderr, "biceps: %s: commande introuvable\n", Mots[0]);
                }
            }
